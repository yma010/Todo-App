...
- **AI scaffolded the Vite project with `npm create vite@latest` and never audited the generated `index.css`.** When I later asked for the layout to span the full viewport and removed `maxWidth: 760` from the React tree, the page still capped at 1126px because the scaffold's `#root { width: 1126px; text-align: center; border-inline: ... }` was silently overriding the layout. The bug survived multiple "fix this" rounds — it wasn't in any test, didn't break type-checking, and only surfaced when I ran the web-interface-guidelines audit, which caught it as a `file:line` finding. Resolved by deleting the dead scaffold rules entirely and writing a small intentional `index.css` from scratch. The lesson: AI-generated boilerplate that doesn't get read is a permanent invisible-bug surface. Audit the scaffold before building on top of it.
- **A second-pass query audit found two cost-per-request issues in the auth path** — not strictly N+1 (no relationship lazy-loading happens anywhere, and the list endpoints each issue exactly one indexed SELECT), but the same "hidden multiplier on every authenticated request" smell. `get_current_user` was issuing two round-trips (`lookup_session` then `db.get(User, session.user_id)`), and `lookup_session` was writing `last_used_at` and committing on *every* authenticated request, turning every read into a write + COMMIT. Folded the user fetch into the session query with `joinedload(SessionRow.user)` so the dependency is a single round-trip, and throttled the `last_used_at` write to once per minute per session via `SESSION_TOUCH_INTERVAL` (audit metadata doesn't need second-level granularity). Net effect: a typical authenticated request is now one SELECT, zero writes — down from two SELECTs plus an UPDATE+COMMIT. All 43 existing tests still pass.
- **AI's first instinct was inline `style={{...}}` for every component, with no `index.css` beyond what Vite scaffolded.** Cleaner-looking at first, but inline styles can't express `:hover`, `:focus-visible`, `transition`, `touch-action`, or `prefers-reduced-motion`. Every interactive element shipped with no visible focus indicator and no hover affordance — caught by the a11y guidelines audit, not by me. Resolved by adding a small global `index.css` with pseudo-state rules that apply across the app (`button:focus-visible`, `button:not(:disabled):hover`, the reduced-motion query), while keeping inline styles for per-component layout. Fewer surfaces to change, and the a11y baseline applies everywhere by default.
- **When asked if we should add full SSR alongside React Query, I declined.** AI would happily have spun up a Vite SSR build with a Node server, dehydrate/hydrate of React Query state, and session-cookie forwarding to the FastAPI backend — multi-hour infrastructure work with real failure modes. But SSR's actual wins (fast first paint, SEO, no FOUC) apply to public/marketing pages; this app is entirely behind auth, where the user already paid the latency to log in and the search engine never indexes anything. The brief explicitly tests scope discipline ("a complete simple solution beats an incomplete ambitious one"). Recorded as a deliberate judgment call rather than a missing feature: scope is a design decision, and saying *no* to an AI-friendly task is harder than saying yes.
- **AI's first take on the deep-linkable tab state used a `useEffect` that watched `tab` and wrote `?tab=` to the URL on every change.** Flagged as a code smell — useEffect mirroring state to a side channel we already control is exactly the anti-pattern the React team's "You Might Not Need an Effect" guidance is about. The tab only changes from user events we own (button click, keyboard nav), so the URL write belongs in the setter, not in an effect that re-subscribes to React's render lifecycle. Refactored into a tiny `useTabState()` hook that returns `[tab, setTab]` with the URL update folded inside `setTab`. useEffect is now reserved for genuine external subscriptions (DOM events, intervals, third-party libraries) — not for syncing state into side channels.
